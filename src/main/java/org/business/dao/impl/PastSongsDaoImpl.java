...
public class PastSongsDaoImpl extends BaseDaoImpl<PastSongs> implements
		IPastSongsDao {
	// 查询一页数据往期歌曲
	@Override
	public PageInfo<PastSongs> selOnePage(PageInfo<PastSongs> page)
		...
		try {
			...
			String dataSQL = "select top "
					+ page.getPageSize()
					+ " sid,fj_root,fj_name,singname,orname from pastsongs where (fj_name like ? or singname like ? or orname like ?) and sid not in( select top "
					+ rows
					+ " sid from pastsongs where (fj_name like ? or singname like ? or orname like ?) order by sid desc) order by sid desc";
